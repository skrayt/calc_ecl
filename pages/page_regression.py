...
def regression_page(page: ft.Page) -> ft.Control:
    ...
    _help = build_help_panel(
        ...
        indicators=[
            ...
            {
                "name": "交差検証（k-fold CV）— 分割数の意味と結果の解釈",
                "criteria": [
                    {"level": "情報",  "range": "5-fold（デフォルト）",              "meaning": "データを5等分し、4/5で学習・1/5でテストを5回繰り返す。一般的な標準設定"},
                    {"level": "注意",  "range": "3-fold（データが少ない場合）",        "meaning": "年次データ15〜20年分など少ない場合は3分割を推奨。各foldの学習データが十分確保できる"},
                    {"level": "情報",  "range": "MSE標準偏差が平均の半分以下",         "meaning": "foldによらず安定した予測精度。モデルが頑健である"},
                    {"level": "注意",  "range": "特定foldだけMSEが突出して大きい",    "meaning": "そのfoldに外れ値・構造変化が含まれる可能性。該当期間を確認する"},
                ],
                "note": "年次データは観測数が少ない（15〜30年分程度）ため、交差検証はあくまで参考指標。過学習のチェックとしてAIC/BICと合わせて活用する",
            },
            {
                "name": "残差プロット — 左・中・右 3枚のグラフの見方",
                "criteria": [
                    {"level": "情報",  "range": "左: 残差 vs 予測値",    "meaning": "残差がランダムに散らばれば良好。U字型・扇型のパターンは非線形性・不均一分散のサイン"},
                    {"level": "情報",  "range": "中: 時系列残差プロット",  "meaning": "時間軸に沿って残差を表示。連続して正または負に偏る場合は自己相関（DW低下）のサイン"},
                    {"level": "情報",  "range": "右: 残差ヒストグラム",    "meaning": "正規分布（釣鐘型）に近いほど良好。大きく歪む場合はデータ変換（対数変換等）を検討する"},
                ],
                "note": "3枚を総合的に判断する。いずれかにパターンが見られる場合は変数追加・変換方法の見直しを検討",
            },
        ],
    )
    return ft.Column(
