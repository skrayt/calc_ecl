...
| Phase 2-1 | indicator_loader（DB→DataFrame） | 未着手 |
| Phase 2-2 | data_transform（データ変換） | 未着手 |
| Phase 2-3 | correlation（相関分析・VIF） | 未着手 |
| Phase 2-4 | regression（OLS回帰） | 未着手 |
...
def transform(df, method) -> pd.DataFrame
    """データ変換。method: none/log/diff/log_diff/arcsinh/arcsinh_diff"""
...
| 2. 相関分析 | `pages/page_correlation.py` | 相関行列ヒートマップ、VIF表 | correlation |
| 3. 回帰分析 | `pages/page_regression.py` | OLS実行、係数表、残差プロット | regression |
| 4. モデル選択 | `pages/page_model_selection.py` | 組み合わせ探索、結果CSV出力 | model_selection |
| 5. ARIMA | `pages/page_arima.py` | ARIMA学習、ACF/PACFプロット | arima |
| 6. 将来シナリオ | `pages/page_forecast.py` | ベース/楽観/悲観作成、DB保存 | db_operations |

### 3-3. 共通コンポーネント

- `components/variable_selector.py`
  - **移植元:** `../Craft_RegressionAnalysis/components/` のVariableSelectorクラス
...
| `src/analysis/regression.py` | `pages/page_regression.py` | `regression_calc()` 関数（交差検証部分） |
| `src/analysis/model_selection.py` | `pages/page_model_selection.py` | `run_analysis()` 内の組み合わせ探索ロジック |
| `components/variable_selector.py` | `components/` | VariableSelectorクラス |
