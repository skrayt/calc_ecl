...
| # | 対象 | 問題 | 修正方針 |
|---|------|------|---------|
| 7A-1 | `pages/page_ecl.py` | Step1エリアに「`Timestamp` object has not subscriptable」エラー | `_load_models()` または `_on_model_select()` 内でTimestampを辞書キーとして添字アクセスしている箇所を修正 |
| 7A-2 | `pages/page_model_confirm.py` | 保存ボタンを複数回押すと同一モデルが重複保存される | 保存前に同一 `(target_variable, feature_variables, dataset_id)` の既存configをチェック。存在すれば保存不可・警告表示 |

---

### 7B: 年次管理の整備

ECL計算は「決算年月ごとに一貫したデータセット」が前提。ARIMA予測・モデル確定に決算年月を紐付ける。

| # | 対象 | 変更内容 |
|---|------|---------|
| 7B-1 | `pages/page_arima.py` | 保存セクションに「対象決算年月」ドロップダウンを追加（indicator_datasetsのfiscal_year_monthから選択） |
| 7B-2 | `pages/page_arima.py` | 同一決算年月・同一変数のARIMAが保存済みの場合、上書き確認ダイアログを表示。または保存一覧に削除ボタンを追加 |
| 7B-3 | `pages/page_model_confirm.py` | 保存時に「対象決算年月」を `model_configs.hyperparameters` JSONBに記録 |
| 7B-4 | `pages/page_model_confirm.py` | 保存済みモデル一覧のID列を非表示（モデル名・目的変数・保存日で識別） |

**DB変更**: なし（`arima_forecasts.scenario_label` に決算年月文字列を格納、または `hyperparameters` JSONB拡張で対応）

---

### 7C: reference_date 期末化 & frequency 包含的取り扱い 【設計変更・要DB再投入】
...
| # | 変更内容 |
|---|---------|
| 7D-1 | 説明変数・目的変数タブに**標準化トグルスイッチ**を追加（変換後プロット用） |
| 7D-2 | グラフサイズ統一（`plot_single_series` に固定 `figsize` パラメータを追加） |
| 7D-3 | 比較プロット: 「2軸表示」トグル廃止 → **「右軸に表示する変数」チェックボックス**方式に変更 |
| 7D-4 | 比較プロットの「0-1正規化（スケール統一）」表記確認（min-max normalizationなので「正規化」が正しい） |
...
| # | 対象 | 変更内容 |
|---|------|---------|
| 7E-1 | `pages/page_regression.py` | 係数テーブルに **VIF列**を追加（`calc_vif(X)` の結果を紐付け） |
| 7E-2 | `pages/page_dynamic_regression.py` | 変数ロード後に各変数の値が `[0,1]` 範囲内か判定。該当変数の標準化トグルを無効化（グレーアウト） |
