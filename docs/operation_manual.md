...
CSVに新しい指標カラム（基準年改定で名前が変わったもの等）が含まれていると、対話プロンプトが表示される。

```
============================================================
未登録のカラムが見つかりました: 「国内総生産（支出側）（名目）2025年基準【10億円】」
============================================================
  自動検出 - 基準年: 2025
  自動検出 - 単位: 10億円

  このカラムを (a)追加 / (s)スキップ対象に追加 / (i)今回だけ無視 ? [a/s/i]:
```

| 選択肢 | 説明 |
|--------|------|
| `a` (追加) | 指標コード等を入力してマッピングに追加。次回以降も自動認識される |
| `s` (スキップ) | このカラムをスキップ対象に登録。次回以降も無視される |
| `i` (今回だけ無視) | 今回だけ無視。次回実行時に再度聞かれる |

追加を選んだ場合、以下を入力する:
- **指標コード**: 英語snake_case（例: `gdp_nominal_2025base`）
- **指標名**: 日本語表示名（自動補完あり）
- **単位**: CSVから自動検出。Enterで確定
- **基準年**: CSVから自動検出。Enterで確定
- **代表粒度**: m=月次 / q=四半期 / c=暦年 / f=年度

入力内容は `config/column_mapping.json` に保存される。
