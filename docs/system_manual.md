...
│   ├── page_arima.py         ← ⑥ ARIMA（予測保存機能付き）
│   ├── page_forecast.py      ← ⑦ 将来シナリオ（参考実装）
│   ├── page_model_confirm.py ← ⑧ モデル確定（回帰モデルDB登録）
│   └── page_ecl.py           ← ⑨ ECL計算（シナリオ加重平均）
├── src/
│   ├── import_indicators.py  ← 説明変数CSVインポート（再掲）
│   ├── import_targets.py     ← 目的変数CSVインポート（再掲）
│   ├── db_operations.py      ← モデル・ARIMA・ECL結果のDB保存/読込
│   ├── data/
│   │   └── indicator_loader.py
│   └── analysis/
│       ├── data_transform.py
...
│       ├── model_selection.py
│       ├── arima.py
│       └── ecl.py            ← ECL計算コアロジック（UI非依存）
├── db/
│   ├── migrations/           ← DDL（テーブル作成・変更SQL）
│   │   ├── 001_create_tables.sql
│   │   ├── 002_create_target_tables.sql
│   │   ├── 003_add_fiscal_year_month.sql
...
[目的変数系]
  target_definitions     ← 目的変数定義（PD/LGD/EAD）
  target_datasets        ← 目的変数データ取得回
  target_data            ← 目的変数データ本体（JSONB格納）
...
[ECL算出系]
  arima_forecasts        ← ARIMA予測結果（指標別・ステップ別）
  ecl_results            ← ECL計算結果（シナリオ加重平均）
```
...
- `indicator_datasets` は**データ取得時期**と**決算年月**を管理
- `model_configs.dataset_id` でモデルとデータセットを紐付け（再現性担保）

### 3.5 決算年月によるデータセット管理

`indicator_datasets.fiscal_year_month` カラムにより、ECLモデルの対象決算期でデータセットを一意に管理する。

- **格納形式**: 月初日のDATE型（例: `2026-03-01` = 2026年3月期）
- **部分ユニークインデックス**: `idx_indicator_datasets_fiscal_ym` により、同一決算年月は1つだけ存在可能（NULLは制約対象外）
- **上書き動作**: 同じ決算年月のCSVを再インポートすると、既存のデータセットと紐づく `indicator_data` を全件DELETEし、データセットレコードも削除した上で新規にINSERTする（DELETE + re-INSERT方式）
- **DDL**: `db/migrations/003_add_fiscal_year_month.sql`

### 3.4 時系列粒度

**Phase 7C以降: reference_date は期末日で格納する。**

| 粒度 | frequency値 | reference_dateルール | CSV上の表記 |
|------|------------|---------------------|------------|
| 月次 | `monthly` | 月末日 `2025-01-31` | `2025年1月` |
| 四半期 | `quarterly` | 四半期末日 `2025-03-31` | `2025年1-3月期` |
| 暦年 | `calendar_year` | 12月31日 `2025-12-31` | `2025年` |
| 年度 | `fiscal_year` | 翌年3月31日 `2026-03-31` | `2025年度` |

期末日格納にすることで、粒度の包含判定が月番号で一意に決まる:
- `monthly` の月 ∈ {3,6,9,12} → `quarterly` の期末と一致
- `monthly` / `quarterly` の月 = 12 → `calendar_year` 末と一致
- `monthly` / `quarterly` の月 = 3 → `fiscal_year` 末と一致

既存データへの適用: `db/migrations/005_update_reference_date_to_period_end.sql` を実行すること。
...
**実行方法**:
```bash
python src/import_indicators.py <CSVパス> [--retrieved-at YYYY-MM-DD] [--fiscal-year-month YYYY-MM-DD]
```

**処理フロー**:
1. `config/column_mapping.json` からカラムマッピングを読み込む
2. CSVに未知のカラムがあれば一括でリスト表示し、対話的にマッピングを追加（JSONに自動保存）
3. `indicator_sources` にデータソース登録（初回のみ）
4. `indicator_definitions` に指標定義を登録（未登録の場合）
5. 決算年月が指定されている場合、同一決算年月の既存データセットを検索
   - 既存データセットが見つかった場合: 紐づく `indicator_data` を全件DELETE → データセットレコードをDELETE
6. `indicator_datasets` にデータセットを新規登録（`fiscal_year_month` 付き）
7. CSVの各行をパースし、`indicator_data` へINSERT

**決算年月による上書き管理**: 同じ決算年月のCSVを再インポートすると、既存データを完全に置き換える（DELETE + re-INSERT方式）。部分ユニークインデックス `idx_indicator_datasets_fiscal_ym` により、同一決算年月のデータセットは常に1つだけ存在する。
...
**未知カラムの検出**: 未知のカラムが複数ある場合、1つずつダイアログを表示するのではなく、全ての未知カラムをまとめて一括表示する。
...
| `list_frequencies(dataset_id)` | 指定データセットに実際に格納されているfrequency一覧を返す |
| `load_indicators(dataset_id, frequency)` | 指標データをDatetimeIndex付きDataFrameで返す（`WHERE frequency = %s` の厳密フィルタ） |
| `get_indicator_definitions(codes)` | 指標定義マスタを取得 |
| `find_dataset_by_fiscal_ym(fiscal_year_month)` | 決算年月からデータセットを検索 |
...
#### 4.4.6 ecl.py — ECL計算コアロジック

| 関数 | 説明 |
|------|------|
| `apply_model_to_forecast(coefficients, forecast_df, feature_stats, transform_method)` | 確定済み回帰モデルにARIMA予測値を代入しPD/LGD予測系列を生成する |
| `calc_weighted_ecl(base_pd, upside_pd, downside_pd, lgd, weight_base, weight_upside, weight_downside, ead)` | 3シナリオの加重平均でECLを計算する |
| `build_scenario_forecast(arima_forecasts_by_var, scenario)` | ARIMA予測結果辞書から指定シナリオのDataFrameを組み立てる |

**`apply_model_to_forecast` の動作**:
1. forecast_dfに変換（`transform_method`）を適用
2. `feature_stats`（保存済みの平均・標準偏差）でZ-score標準化
3. 切片 + Σ(係数 × 説明変数) で線形結合を計算し pd.Series を返す

**標準化reverse-transformの設計**:
- モデル確定時、標準化ON の場合に `feature_stats = {"変数名": {"mean": ..., "std": ...}}` を `hyperparameters` JSONBに保存
- ECL計算時にこれを読み出して同じ標準化を適用することで、学習時と同じスケールで予測できる

### 4.5 src/db_operations.py — モデル・ARIMA・ECL結果のDB保存/読込

UI側からDB保存・読込を行う関数群。`config.db.get_connection()` を使い、`try/finally` でコネクションをクローズする。

**モデル管理**:

| 関数 | 説明 |
|------|------|
| `save_model_config(config_dict) -> int` | model_configs にINSERTし、config_id を返す |
| `save_model_result(config_id, result_dict) -> int` | model_results にINSERTし、result_id を返す |
| `load_model_configs() -> pd.DataFrame` | 保存済みモデル一覧を返す |
| `load_model_result(config_id) -> dict` | 指定config_idの最新model_resultsを返す |
| `delete_model_config(config_id)` | model_configs と紐づくmodel_resultsを削除する |

**ARIMA予測管理**:

| 関数 | 説明 |
|------|------|
| `save_arima_forecast(forecast_dict) -> int` | arima_forecasts にINSERTし、forecast_id を返す |
| `load_arima_forecasts(indicator_code=None, dataset_id=None) -> pd.DataFrame` | 保存済みARIMA予測一覧を返す（indicator_code / dataset_id でフィルタ可） |
| `load_arima_forecast_data(forecast_id) -> pd.DataFrame` | forecast_data JSONBをDataFrameに変換して返す |
| `delete_arima_forecast(forecast_id)` | ARIMA予測結果を削除する |

**ECL結果管理**:

| 関数 | 説明 |
|------|------|
| `save_ecl_result(ecl_dict) -> int` | ecl_results にINSERTし、ecl_id を返す |
| `load_ecl_results(fiscal_year_month=None) -> pd.DataFrame` | 保存済みECL計算結果一覧を返す（決算年月フィルタ可） |
| `list_ecl_fiscal_year_months() -> list` | 保存済みECL結果の決算年月一覧を返す |
| `delete_ecl_result(ecl_id)` | ECL計算結果を削除する |
| `load_ecl_result_detail(ecl_id) -> list` | ECL結果の results JSONB を返す |

### 4.7 GUIアプリケーション（Flet）
...
| 3 | 回帰分析 | `pages/page_regression.py` | データソース選択・OLS回帰・係数テーブル（VIF列付き）・残差プロット・交差検証 |
| 4 | モデル選択 | `pages/page_model_selection.py` | データソース選択・全組み合わせ探索・進捗バー・VIFフィルタ |
| 5 | 動的回帰 | `pages/page_dynamic_regression.py` | データソース選択・変数別変換・標準化・ラグ設定（[0,1]範囲変数の標準化トグル自動無効化） |
| 6 | ARIMA | `pages/page_arima.py` | ADF検定・ACF/PACF・次数自動選択・予測・予測結果DB保存（初期表示時にデータセット全変数の保存済み一覧を表示） |
| 7 | 将来シナリオ | `pages/page_forecast.py` | シナリオ作成・予測結果保存（参考実装） |
| 8 | モデル確定 | `pages/page_model_confirm.py` | 回帰モデルのDB登録・保存済みモデル一覧・削除 |
| 9 | ECL計算 | `pages/page_ecl.py` | モデル選択・ARIMA予測割当・シナリオウェイト設定・ECL計算・CSV出力（決算年月自動設定・保存済み一覧・詳細表示） |
...
| `components/variable_selector.py` | 目的変数・説明変数の選択UIパーツ。`unit_range_columns` 引数で全値が[0,1]の変数を指定すると標準化トグルを自動的に `disabled=True` にする |
...
### 4.8 config/column_mapping.json — カラムマッピング設定
...
```
001_create_tables.sql              ← 初期テーブル作成（説明変数・モデル系 7テーブル）
002_create_target_tables.sql       ← 目的変数テーブル作成（target_* 3テーブル）
003_add_fiscal_year_month.sql      ← indicator_datasetsに決算年月カラム追加
004_create_ecl_tables.sql          ← ECL算出テーブル作成（arima_forecasts・ecl_results）
005_update_reference_date_to_period_end.sql ← 既存データのreference_dateを期初→期末に一括変換（Phase 7C）
```
