...
| `list_frequencies(dataset_id)` | 指定データセットで利用可能なfrequency一覧（**包含ロジック適用**。格納粒度より粗い粒度も展開して返す） |
| `load_indicators(dataset_id, frequency)` | 指標データをDatetimeIndex付きDataFrameで返す（**包含クエリ**。格納粒度より粗いfrequencyを指定した場合、期末月番号一致で細粒度レコードを包含。同一日付は粗い粒度を優先） |
| `get_indicator_definitions(codes)` | 指標定義マスタを取得 |
| `load_dataset_summary(dataset_id)` | データセットの概要情報を取得 |

**包含ロジックの例**: データセットに `monthly` データしか存在しない場合でも、`list_frequencies()` は `['monthly', 'quarterly', 'calendar_year', 'fiscal_year']` を返す。`load_indicators(dataset_id, 'fiscal_year')` を呼ぶと `monthly` レコードのうち月=3のものが返される。
...
| `load_arima_forecasts(indicator_code=None) -> pd.DataFrame` | 保存済みARIMA予測一覧を返す（indicator_code指定でフィルタ可） |
| `load_arima_forecast_data(forecast_id) -> dict` | forecast_data JSONBを返す |
...
| `load_ecl_results() -> pd.DataFrame` | 保存済みECL計算結果一覧を返す |
...
| 6 | ARIMA | `pages/page_arima.py` | ADF検定・ACF/PACF・次数自動選択・予測・予測結果DB保存 |
| 7 | 将来シナリオ | `pages/page_forecast.py` | シナリオ作成・予測結果保存（参考実装） |
| 8 | モデル確定 | `pages/page_model_confirm.py` | 回帰モデルのDB登録・保存済みモデル一覧・削除 |
| 9 | ECL計算 | `pages/page_ecl.py` | モデル選択・ARIMA予測割当・シナリオウェイト設定・ECL計算・CSV出力 |
